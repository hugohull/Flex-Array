...
private:
	...
	// Shifts array to the right or left
	void shift(int i, int v);

	// Checks to see if the array needs resizing
	void over_limit();

	// Returns the true capacity of the array. Not necessary but saves typing and make it easy me to visualise
	int true_cap();

	// Calculates the middle of the capacity using the true_cap method
	int mid_cap();
